...
export const createNavigation = (navigationRouteName: string) => ({
  ...
  reset: (
    routeName?: string | ([string] | [string, NavigationParams])[],
    params?: NavigationParams,
  ) => {
    const navigator = navigatorsByName[navigationRouteName];
    if (navigator != null) {
  ...
  getState: () => navigatorsByName[navigationRouteName]?.state.nav,
});
